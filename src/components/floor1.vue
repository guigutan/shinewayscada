<template>
      ...
      <tpTotal :dataListTotalSum="dataListMachineLedStatus" :loading="loading" />
</template>
...
<script setup lang="ts">
          ...
          const area = ref("一楼")
          const ScadaNO=ref(Number (dayjs(Date.now()).format('YYYYMMDDHHmm')))
          const title = ref("一楼CNC机台设备")
          const currentPath = ref("/floor1") 
          const Msg = ref("这里是机台动态信息") 
          const dataListTotalSum = ref<TotalSumResult>()

          onMounted(async () => {
            loading.value = true
            try {
                  ScadaNO.value=Number (dayjs(Date.now()).format('YYYYMMDDHHmm'))
                  dataListMachineLedStatus.value = await loadDataMachineLedStatus(ScadaNO.value,area.value)
                  dataListTotalSum.value=await loadDataTotal(ScadaNO.value,ScadaNO.value,area.value)
